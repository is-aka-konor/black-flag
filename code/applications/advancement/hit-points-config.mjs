import AdvancementConfig from "./advancement-config.mjs";

/**
 * Configuration application for hit points.
 */
export default class HitPointsConfig extends AdvancementConfig {
	/** @override */
	static DEFAULT_OPTIONS = {
		classes: ["hit-points", "form-list"]
	};

	/* <><><><> <><><><> <><><><> <><><><> */

	static PARTS = {
		config: {
			template: "systems/black-flag-ru/templates/advancement/advancement-controls-section.hbs"
		},
		hitDice: {
			template: "systems/black-flag-ru/templates/advancement/hit-points-config.hbs"
		}
	};

	/* <><><><> <><><><> <><><><> <><><><> */
	/*              Rendering              */
	/* <><><><> <><><><> <><><><> <><><><> */

	/** @inheritDoc */
	async _prepareContext(options) {
		const context = await super._prepareContext(options);
		context.hitDieSizes = CONFIG.BlackFlag.hitDieSizes.map(value => ({ value, label: `d${value}` }));
		context.showClassRestriction = false;
		return context;
	}
}
